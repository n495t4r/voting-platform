...
<div class="mb-6">
<h1 class="text-3xl font-bold text-gray-900">Edit Election</h1>
<p class="text-gray-600">Update the details for {{ $election->title }}</p>
</div>
...
<div class="card-mobile">
    <form method="POST" action="{{ route('admin.elections.update', $election) }}" class="space-y-6">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4" class="input-mobile w-full">{{ old('description', $election->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="datetime-local" id="starts_at" name="starts_at"
                   value="{{ old('starts_at', $election->starts_at->format('Y-m-d\TH:i')) }}"
                   required class="input-mobile w-full">
            @error('starts_at')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="datetime-local" id="ends_at" name="ends_at"
                   value="{{ old('ends_at', $election->ends_at->format('Y-m-d\TH:i')) }}"
                   required class="input-mobile w-full">
            @error('ends_at')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
