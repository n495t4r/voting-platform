        ...
        <form method="POST" action="{{ route('admin.elections.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="title" name="title" value="{{ old('title') }}" 
                       ...
                       placeholder="e.g., 2024 Alumni Association Elections">
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" class="input-mobile w-full"
            ...
            <div>
                ...
                <input type="datetime-local" id="starts_at" name="starts_at" 
                       value="{{ old('starts_at') }}" required class="input-mobile w-full">
                @error('starts_at')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" id="ends_at" name="ends_at" 
                       value="{{ old('ends_at') }}" required class="input-mobile w-full">
                @error('ends_at')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
